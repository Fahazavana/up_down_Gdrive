...
class Drive():
    ...
    def list_files(self, folder_id):
        # Call API
        response = self.__service.files().list(q="'{}' in parents".format(
            folder_id), fields='files(id,name,modifiedTime,mimeType)').execute()
    ...
    def download_file(self, filename, local_path, file_id, update=False):
        ...
        modified_time = self.__service.files().get(
            fileId=file_id, fields='modifiedTime').execute()['modifiedTime']
        ...
        if update != False:
            print("\n[<<++]Local file '{}' updated successfully in folder '{}'.\n".format(
                filename, local_absolute_path.rsplit('/', 2)[-2]))
        else:
    ...
    def upload_file(self, filename, local_path, folder_id, update=False):
        ...
        modified_timestamp = Utils.get_local_file_timestamp(
            local_absolute_path)
        file_metadata = {'name': filename, 'modifiedTime': Utils.convert_timestamp_datetime(
            modified_timestamp), 'parents': [folder_id]}
        ...
        try:
            ...
            if extension not in self.ignore:
                ...
                if update != False:
                    uploaded_file = self.__service.files().update(
                        fileId=update, media_body=media).execute()
                ...
                print("[*] {} is in the ignore file".format(filename))
        except:
    ...
    def upload_folder(self, foldername, folder_id):
        ...
        folder_metadata = {'name': foldername, 'parents': [
            folder_id], 'mimeType': 'application/vnd.google-apps.folder'}

        try:
            # Send POST request for upload API
            uploaded_folder = self.__service.files().create(body=folder_metadata).execute()
            print('\n++ Remote folder created: {}\n'.format(
                uploaded_folder['name']))
    ...
    def synchronize(self, local_path, folder_id):
        ...
        for sm_file in same_files:
            ...
            if modified == 'local':
                if os.path.isdir(local_absolute_path):
                    ...
                    self.upload_file(sm_file, local_path,
                                     folder_id, remote_file_data['id'])
                ...
                if remote_file_data["mimeType"] == 'application/vnd.google-apps.folder':
                    ...
                    self.download_file(sm_file, local_path,
                                       remote_file_data['id'], True)
        ...
        for diff_file in different_files:
                ...
                for remote_file in drive_files['all']:
                    if remote_file['name'] == diff_file:
                        if remote_file['mimeType'] == 'application/vnd.google-apps.folder':
                            local_absolute_path = "{}/{}".format(
                                local_path, diff_file)
                            # Recursive to download files inside folders
                            self.synchronize(
                                local_absolute_path, remote_file['id'])
                        else:
                            self.download_file(
            ...
            # IF file is only on local (UPLOAD)
                ...
                if os.path.isdir(local_absolute_path):
                    created_folder_id = self.upload_folder(
                        diff_file, folder_id)
...
def main():
    ...
    my_drive = Drive()
    my_drive.synchronize(CONFIGS['local_folder_path'],
                         CONFIGS['drive_folder_id'])
